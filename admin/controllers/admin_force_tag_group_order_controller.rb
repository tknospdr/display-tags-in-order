...
module Admin
  class ForceTagGroupOrderController < Admin::AdminController
    ...
    def index
      ...
      selected_order = SiteSetting.force_tag_group_order.split(',').map(&:strip).reject(&:empty?)
      render_json_dump(tag_groups: tag_groups, selected_order: selected_order)
    end

    def update
      new_order = params[:order]
      if new_order.is_a?(Array) && new_order.all? { |name| name.is_a?(String) }
        SiteSetting.force_tag_group_order = new_order.join(',')
